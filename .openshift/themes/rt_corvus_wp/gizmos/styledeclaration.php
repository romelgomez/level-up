...
class GantryGizmoStyleDeclaration extends GantryGizmo {
	...
	function query_parsed_init() {
		...
		// Less Variables
		$lessVariables = array(
            'main-background'  =>$gantry->get('main-background', '#D5CDCB'),
            'main-body'        =>$gantry->get('main-body',       'light'),
            'main-accent'      =>$gantry->get('main-accent',     '#009BDD')
        );

        $positions = array('top-customtop', 'bottom-custombottom');
		$source = "";
	...
	function buildLogo(){
		...
		if ($gantry->get('logo-type')!="customlogo") return "";

		$source = $width = $height = "";

		$logo = str_replace("&quot;", '"', str_replace("'", '"', $gantry->get('logo-customlogo-image')));
