<?php
/**
 * @version   $Id: Abstract.php 14373 2013-10-09 22:49:38Z btowles $
...
abstract class RokSprocket_Provider_Simple_Storage_Abstract implements RokSprocket_Provider_Simple_Storage_Interface
{
	...
	public function removeItem($item_id, $module_id)
	{
		...
		RokCommon_Session::set('roksprocket.' . $module_id, serialize($items));
		return true;
	}
	...
	public function getItems($module_id, $displayedIds = array())
	{
		...
		if ($this->isAdmin()) {
			$items = RokCommon_Session::get('roksprocket.' . $module_id, false);
			if ($items === false) {
				$items = RokCommon_Session::set('roksprocket.' . $module_id, serialize($this->getItemsFromDB($module_id)));
			}
