<?php
/**
 * @version   $Id: roksprocket.php 11853 2013-06-28 21:21:58Z btowles $
...
if (!defined('ROKSPROCKET_NEEDED_ROKCOMMON_VERSION')) define('ROKSPROCKET_NEEDED_ROKCOMMON_VERSION', '3.1.6');
...
if (!defined('ROKSPROCKET_VERSION')) define('ROKSPROCKET_VERSION', '2.0.5');
if (!defined('ROKSPROCKET_PLUGIN_FILE')) define('ROKSPROCKET_PLUGIN_FILE', str_replace(WP_PLUGIN_DIR, '', $plugin));
if (!defined('ROKSPROCKET_PLUGIN_REL_PATH')) define('ROKSPROCKET_PLUGIN_REL_PATH', '/' . str_replace(array(
                                                                                                          '/',
                                                                                                          '\\'
                                                                                                     ), '', dirname(ROKSPROCKET_PLUGIN_FILE)));
...
if (!preg_match('/project.version/', ROKCOMMON) && version_compare(preg_replace('/-SNAPSHOT/', '', ROKCOMMON), ROKSPROCKET_NEEDED_ROKCOMMON_VERSION, '<')) {
	rs_force_deactivate();
	roksprocket_set_admin_message('error', __('RokSprocket v2.0.5 needs at least RokCommon Version ' . ROKSPROCKET_NEEDED_ROKCOMMON_VERSION . '.  You currently have RokCommon Version ' . ROKCOMMON));
	return;
}
	...
	class RokSprocket_Plugin
	{
		...
		public static function init()
		{
			...
			if ($container->hasParameter('roksprocket.template.override.path')) {
				load_plugin_textdomain('wp_roksprocket', false, ROKSPROCKET_PLUGIN_REL_PATH . '/languages');
		...
		public static function admin_menu()
		{
			...
			add_menu_page('RokSprocket Administration', 'RokSprocket Admin', 'manage_options', 'roksprocket-list', array(
			                                                                                                            'RokSprocket_Plugin',
			                                                                                                            'renderView'
			                                                                                                       ), ROKSPROCKET_PLUGIN_URL . '/admin/assets/images/roksprocket_16x16.png');
			add_submenu_page(null, 'RokSprocket Edit Widget', 'RokSprocket Edit Widget', 'manage_options', 'roksprocket-edit', array(
			                                                                                                                        'RokSprocket_Plugin',
			                                                                                                                        'renderView'
			                                                                                                                   ));
			add_submenu_page('roksprocket-list', 'Add New', 'Add New', 'manage_options', 'roksprocket-edit', array(
			                                                                                                      'RokSprocket_Plugin',
			                                                                                                      'renderView'
			                                                                                                 ));
		}
		...
		function roksprocket_admin_bar()
		{
			...
			$args  = array(
				...
				'href'   => get_bloginfo('wpurl') . '/wp-admin/admin.php?page=roksprocket-edit&id=0&nonce=' . $nonce,
			);
...
if (!function_exists('roksprocket_activate')) {
	...
	function roksprocket_activate()
	{
		...
		if (!in_array($include_file, $included_files) && ($loaderrors = include_once($include_file)) !== true) {
			rs_force_deactivate();
			roksprocket_set_admin_message('error', __('<strong>Unable to activate RokSprocket:</strong><br/>') . implode("<br/>", $loaderrors));
		}
...
if (!function_exists('roksprocket_mootools_init')) {
	...
	function roksprocket_mootools_init()
	{
		...
		if ((is_admin() && $pagenow == 'widgets.php' && !$gantry) || (is_admin() && $pagenow == 'admin-ajax.php') || (is_admin() && $page == 'roksprocket-list') || (is_admin() && $page == 'roksprocket-edit')
		) {
