<?php
/**
 * @version $Id: tinymce.php 10888 2013-05-30 06:32:18Z btowles $
...
class RokSprocket_TinyMCE
{
    ...
    function roksprocket_quicktags()
    {
        ...
        if (version_compare($wp_version, '3.1.0', '<')) {
            RokCommon_Header::addScript(RokCommon_Composite::get('rs_tinymce')->getURL('quicktags.js'));
        } else {
                    ...
                    if($('qt_content_roksprocket')) {
                        $('qt_content_roksprocket') . addEvent('mouseover', function ()
                        {
                            if ($('roksprocket_link') == null) {
                                var url = ajaxurl + '?action=roksprocket_tinymce&type=qtags&TB_iframe=true&height=150&width=500&modal=false';
